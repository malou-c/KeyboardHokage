...
MyKeyboard::MyKeyboard(int x, int y)
{
    ...
    wchar_t code_line1[num_in_line[0]][count_code] = {{L'`', L'~', L'ё', L'Ё'},
                                                      ...
                                                      {L'-', L'_', L'-', L'_'},
                                                      {L'=', L'+', L'=', L'+'}};
    wchar_t code_line2[num_in_line[1]][count_code] = {{L'q', L'Q', L'й', L'Й'},
                                                      {L'w', L'W', L'ц', L'Ц'},
                                                      {L'e', L'E', L'у', L'У'},
                                                      {L'r', L'R', L'к', L'К'},
                                                      {L't', L'T', L'е', L'Е'},
                                                      ...
                                                      {L'[', L'[', L'х', L'Х'},
                                                      {L']', L']', L'ъ', L'Ъ'}};
    wchar_t code_line3[num_in_line[2]][count_code] = {{L'a', L'A', L'ф', L'Ф'},
                                                      {L's', L'S', L'ы', L'Ы'},
                                                      {L'd', L'D', L'в', L'В'},
                                                      {L'f', L'F', L'а', L'А'},
                                                      {L'g', L'G', L'п', L'П'},
    ...
    for (size_t num = 0; num < count_code; num++) {
        for (int i = start_fill_but, j = 0; j < num_in_line[num]; i++, j++) {
            ...
            vec_buttons[i].sprite.setTextureRect(
                    IntRect(j * tile_size.x,
                            num * tile_size.y,
                            tile_size.x,
                            tile_size.y));
