...
MenuButton::MenuButton(int x, int y, std::string button_name)
{
...
int MenuButton::is_clicked(sf::RenderWindow &window) //Проверяет была ли нажата кнопка
{
    if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && sf::IntRect((background).getLocalBounds()).contains(sf::Mouse::getPosition(window)))
    {
        return id;
    }
    else
